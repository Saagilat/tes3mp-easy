...
main() {
    ...
    if [[ -d "$DEST/players" ]] && ls "$DEST/players/"*.json &>/dev/null 2>&1; then
        read -r -p "Export player data before removal? [y/N]: " ans
        case "${ans,,}" in y|yes)
    ...
    if [[ -d "$DEST/world" ]] && ls "$DEST/world/"*/*.json &>/dev/null 2>&1; then
        read -r -p "Export world data before removal? [y/N]: " ans
        case "${ans,,}" in y|yes)
    ...
    if [[ -d "$DEST/mods/plugins" ]] || [[ -d "$DEST/mods/scripts" ]]; then
        read -r -p "Export mods before removal? [y/N]: " ans
        case "${ans,,}" in y|yes)
            info "Exporting mods..."
    ...
    if command -v ufw &>/dev/null && ufw status 2>/dev/null | grep -q "active"; then
        echo ""
        read -r -p "Remove TES3MP ports (25565/udp, 8085/tcp) from UFW? [y/N]: " ans
        case "${ans,,}" in y|yes)
    ...
    if command -v firewall-cmd &>/dev/null && firewall-cmd --state 2>/dev/null | grep -q "running"; then
        echo ""
        read -r -p "Remove TES3MP ports from firewalld? [y/N]: " ans
        case "${ans,,}" in y|yes)
